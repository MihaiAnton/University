...
public class Service {
    ...
    public int getStyleCount(Style style, RaceLength raceLength){
        int count = 0;
        for (Registration r:registrationRepo.findAll()) {
            //System.out.println(r.getStyle() + " " + r.getRaceId().getFirst());
            if(r.getRaceId().getFirst().equals(raceLength) && r.getRaceId().getSecond().equals(style)){
                count++;
            }
        }
        return count;
     ...
     * @return
     */
    public int addRacesToAthlete(int athleteId, RaceType raceType){
        Athlete athlete = athleteRepo.findOne(athleteId);
        if(athlete == null){
    ...
    public Iterable<AthleteRaceDTO> getAthletesRaceDTO() {
        List<AthleteRaceDTO> list = new ArrayList<>();

        for (Registration r: registrationRepo.findAll()) {
            Athlete athlete = athleteRepo.findOne(r.getAthleteId());
            RaceType raceType = raceTypeRepo.findOne(r.getRaceId());
            list.add(new AthleteRaceDTO(athlete,raceType));
        }
        return list;
    }
}
